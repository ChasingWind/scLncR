...
spec <- matrix(
    c("sample_dirs",  "d", 2, "character", "The dirs contant count matrix of samples, you should use the absolute path. \
                                            If you have multiple sample data, you can use commas to separate them.",
      "sample_names",  "s", 2, "character",  "Sample names will be used as the project name in Seurat to creat seurat object.\
                                              If you have multiple sample data, you can use commas to separate them",
...
data_process <- function(seu_obj, res_dir=""){
    dir.create(res_dir)
    setwd(res_dir)
    getwd()
    ...
    lnc_trans <- grep("^test.*", seu_obj.markers$gene)
...
pipe <- function(seu_obj, sample_name){
    ...
    seu_lnc <- cut_cell_matrix(data_matrix=df_count, pattern="^test.*", project_name=paste(sample_name, "lnc", sep="_"))
...
if( !is.null(opt$help) || is.null(opt$sample_dirs) || is.null(opt$sample_names) ){
...
sample_dirs <- strsplit(opt$sample_dirs, ",")[[1]]
sample_names <- strsplit(opt$sample_names, ",")[[1]]
res_dir <- opt$output_path
print(sample_dirs)
pipe_merge(sample_dirs=sample_dirs, project_names=sample_names, res_dir=res_dir)